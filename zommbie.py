...
def peashootHasHitZombie(peashootRect, zombies):
    for z in zombies:
        if peashootRect.colliderect(z['rect']):
            return True
    return False
def sunflowerHasHitZombie(sunflowerRect, zombies):
    for z in zombies:
        if sunflowerRect.colloderect(z['rect']):
            return True
    return False
...
# set up images
peashootImage = pygame.image.load('SnowPea.gif')
peashootRect = peashootImage.get_rect()

sunflowerImage = pygame.image.load('sunflower.png')
sunflowerRect = sunflowerImage.get_rect()
...
windowSurface.blit(peashootImage, (WINDOWWIDTH / 2, WINDOWHEIGHT - 70))
windowSurface.blit(sunflowerImage,(WINDOWWIDTH / 2,WINDOWHEIGHT -70))
drawText('Zombie Defence By Handsomestone', font, windowSurface, (WINDOWWIDTH / 4), (WINDOWHEIGHT / 4))
drawText('Shed Of The Dead', font, windowSurface, (WINDOWWIDTH / 3), (WINDOWHEIGHT / 3))
...
while True:
    ...
    peashootRect.topleft = (50, WINDOWHEIGHT /2)
    moveLeft = moveRight = False
    moveUp=moveDown = False
    shoot = False

    sunflowerRect.topleft = (50,WINDOWHEIGHT/2)
    moveLeft = moveRight = False
    moveUp=moveDown = False
    shoot = False
    ...
    while True: # the game loop runs while the game part is playing
        ...
        if moveUp and peashootRect.top > 30:
            peashootRect.move_ip(0,-1 * PEASHOOTMOVERATE)
        if moveDown and peashootRect.bottom < WINDOWHEIGHT-10:
            peashootRect.move_ip(0,PEASHOOTMOVERATE)

        if moveUp and sunflowerRect.top > 30:
            sunflowerRect.move_ip(0,-1 * SUNFLOWERMOVERATE)
        if moveDown and sunflowerRect.bottom < WINDOWHEIGHT-10:
            sunflowerRect.move_ip(0,SUNFLOWERMOVERATE)
        ...
        windowSurface.blit(peashootImage, peashootRect)
        windowSurface.blit(sunflowerImage, sunflowerRect)
        # Draw each baddie
        for z in zombies:
        ...
        if peashootHasHitZombie(peashootRect, zombies):
            break
        if peashootHasHitZombie(peashootRect, newKindZombies):
           break

        if sunflowerHasHitZombie(sunflowerRect, zombies):
            break
        if sunflowerHasHitZombie(sunflowerRect, newKindZombies):
           break
    ...
    if zombiesGottenPast >= MAXGOTTENPASS:
        ...
        drawText('YOUR COUNTRY HAS BEEN DESTROYED BY ZOMBIES', font, windowSurface, (WINDOWWIDTH / 4)- 80, (WINDOWHEIGHT / 3) + 100)
    ...
    if peashootHasHitZombie(peashootRect, zombies):
        windowSurface.blit(rescaledBackground, (0, 0))
        windowSurface.blit(peashootImage, (WINDOWWIDTH / 2, WINDOWHEIGHT - 70))
    if sunflowerHasZombie(sunflowerRect, zombie):
        windowSurface.blit(playerImage, (WINDOWWIDTH / 2, WINDOWHEIGHT - 70))
        drawText('score: %s' % (score), font, windowSurface, 10, 30)
        drawText('GAME OVER', font, windowSurface, (WINDOWWIDTH / 3), (WINDOWHEIGHT / 3))
        drawText('YOU HAVE BEEN KISSED BY THE MENTAL FLOSS', font, windowSurface, (WINDOWWIDTH / 4) - 80, (WINDOWHEIGHT / 3) +100)
        drawText('Press enter to play again or escape to exit', font, windowSurface, (WINDOWWIDTH / 4) - 80, (WINDOWHEIGHT / 3) + 150)
        pygame.display.update()
        waitForPlayerToPressKey()
    gameOverSound.stop()